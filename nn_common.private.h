...
static void __setup_augmented_features(struct indigo_nn* nn, int last_action)
{
    ...
    // [NUM_FEATURES, NUM_FEATURES + NUM_ACTIONS) of the input vector are "augmented features"
    ...
    memset(input_vec + NUM_FEATURES, 0, sizeof(float) * NUM_ACTIONS);
    input_vec[NUM_FEATURES + last_action] = 1.0;
}
