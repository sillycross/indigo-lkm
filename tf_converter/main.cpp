...
public:
    void Convert(std::string className)
    {
        ...
        printf("static inline float* %s_get_address_for_argument(struct %s* nn, int index) {\n", className.c_str(), className.c_str());
        ...
        printf("static inline int %s_get_argument_size_bytes(int index) {\n", className.c_str());
        ...
        printf("static inline float* %s_get_address_for_result(struct %s* nn, int index) {\n", className.c_str(), className.c_str());
        ...
        int headerSize = sizeof(void*) * kNumBuffers;
        printf("static inline int WARN_UNUSED %s_constructor(struct %s* nn) {\n", className.c_str(), className.c_str());
        printf("\tint i;\n\tuintptr_t start;\n");
        printf("\tnn->m_bufferList = (void**)kmalloc(%sbufferLength + %d, GFP_NOWAIT);\n", prefix.c_str(), headerSize + kAlign);
        printf("\tif (nn->m_bufferList == NULL) { return 0; }\n");
        printf("\tstart = ((uintptr_t)(nn->m_bufferList)) + %d;\n", headerSize);
        int log2kAlign = __builtin_ctz(kAlign);
        assert((1 << log2kAlign) == kAlign);
        printf("\tstart = (start + %d) >> %d << %d;\n", kAlign - 1, log2kAlign, log2kAlign);
        printf("\tfor (i = 0; i < %snumBuffers; i++) {\n", prefix.c_str());
        printf("\t\tif(%sbufferOffsets[i] == -1) {\n\t\t\tnn->m_bufferList[i] = NULL;\n", prefix.c_str());
        printf("\t\t} else {\n\t\t\tnn->m_bufferList[i] = ((char*)start) + %sbufferOffsets[i];\n\t\t}\n\t}\n", prefix.c_str());
        printf("\treturn 1;\n}\n\n");

        printf("static inline void %s_destructor(struct %s* nn) {\n", className.c_str(), className.c_str());
        printf("\tAssert(nn->m_bufferList != NULL);\n");
        printf("\tkfree(nn->m_bufferList);\n\tnn->m_bufferList = NULL;\n}\n\n");

        printf("static inline void %s_run(struct %s* nn) {\n", className.c_str(), className.c_str());
        printf("\tAssert(nn->m_bufferList != NULL);\n");
